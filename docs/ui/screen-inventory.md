...
- `Graph Explorer` is built. It renders every non-archived knowledge node and the links between them as a dependency-free SVG map (no graph library, no physics; positions come from a deterministic layout computed during render, so the server HTML already contains every mark and edge). Verification is encoded twice, by colour and by shape, with a legend. A mark is a link: clicking or pressing Enter opens the node at `/tree/node/:id`, and hover or keyboard focus opens a short content preview card fetched from the node preview endpoint. Filters narrow the map by branch and by node title. The same component renders the 1-hop local map on `Node Detail`, which also lists that node's outgoing links and its backlinks.
