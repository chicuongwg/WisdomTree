...
- [`../tests/README.md`](../tests/README.md): executable verification layers, commands, and database prerequisites for the current repository.
...
- [`diagrams/`](./diagrams/): draw.io presentation copies of the architecture, lifecycle, ERD, topology, code structure, verification gates, and flow figures. The Markdown docs are canonical; generated `.drawio` files must be regenerated with `node scripts/gen-diagrams.mjs` when their source changes.
- [`diagrams/code-and-test-architecture.drawio`](./diagrams/code-and-test-architecture.drawio): current facade/slice structure and the ordered `test:all` release gate.
