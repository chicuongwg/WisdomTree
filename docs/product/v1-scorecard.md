...
| `weekly_active_accounts_by_role` | Unique active accounts in the trailing 7 days, reported by role group. Count an account as active when it performs at least one meaningful read, search, upload, edit, correction, review, or publish action. | Auth events plus audit or interaction events. | Weekly | `>= 50%` of enabled user-capable accounts and `>= 50%` of enabled editor-capable accounts are active. If a role group has fewer than 2 enabled accounts, one active account counts as green. | Either role group is `25% - 49%` active. | Either role group is `< 25%` active. |
