...
export function ResearchSearchView({
  ...
  return (
    <PageContainer width="wide">
      <Stack>
        <PageHeader title={translate(locale, "nav.search")} description={translate(locale, "search.description")} />
        <form className="ui-next-inline" method="get">
          <label style={{ flex: 1 }}>
            <span className="ui-next-visually-hidden">{translate(locale, "nav.search")}</span>
            <input className="ui-next-control" name="q" defaultValue={query} placeholder={translate(locale, "search.placeholder")} maxLength={200} />
          </label>
          <select className="ui-next-control" name="type" defaultValue={type}>
            {types.map((value) => <option key={value} value={value}>{value === "all" ? translate(locale, "search.filter.all") : translate(locale, `shell.kind.${value}`)}</option>)}
          </select>
          <button className="ui-next-button ui-next-button--primary" type="submit">{translate(locale, "nav.search")}</button>
        </form>
        {!query ? <Surface><p>{translate(locale, "search.empty")}</p></Surface> : null}
        {query && !results.length ? <Surface><p>{translate(locale, "search.noResults")}</p></Surface> : null}
        {results.length ? (
          <ul className="ui-next-project-list" aria-label={translate(locale, "shell.searchResults")}>
            {results.map((result) => (
              <li key={`${result.kind}:${result.id}`}>
                <Surface className="ui-next-project-card">
                  <div className="ui-next-project-card__main">
                    ...
                    {result.summary ? <p className="ui-next-project-card__description" dir="auto">{result.summary}</p> : null}
                    <p className="ui-next-muted">
                      {result.kind === "person" ? result.projects.map((project) => project.name).join(" · ") : result.project.name}
                    </p>
                  </div>
                </Surface>
              </li>
